...
def add_missing_motif_type(df, mtype, signed_motif_types):
	if len(mtype) < len(signed_motif_types):
		...
		df['binary_count'] = pd.to_numeric(df['binary_count'])
	return df, mtype

def count_motifs_using_matrix(motif_type, G):
	adj_matrix = nx.to_numpy_array(G)
	# motif_types = ['021D', '021U', '021C', '111D', '111U', '030T', '030C', '201', '120D', '120U', '120C', '210',
	# 			   '300']
	adj_matrix = np.abs(adj_matrix) # for now
	adj_matrix_bin = (adj_matrix != 0).astype(int)
	intensity = 0
	binary_count = 0
	match motif_type:
		case '021D':
		...
		case '021C':
			pass
		case '111D': # A^3
			intensity = np.trace(adj_matrix @ adj_matrix @ adj_matrix)
			binary_count = np.trace(adj_matrix_bin @ adj_matrix_bin @ adj_matrix_bin)
			print('111D here!!!!!!!!!!!!!!!!!!!!!!!!!!')
		case '111U': # A^T A^2
			intensity = np.trace(adj_matrix.T @ adj_matrix @ adj_matrix)
			binary_count = np.trace(adj_matrix_bin.T @ adj_matrix_bin @ adj_matrix_bin)
			print('111U here!!!!!!!!!!!!!!!!!!!!!!!!!!')
		case '030T': # A A^T A
			intensity = np.trace(adj_matrix @ adj_matrix.T @ adj_matrix)
			binary_count = np.trace(adj_matrix_bin @ adj_matrix_bin.T @ adj_matrix_bin)
			print('030T here!!!!!!!!!!!!!!!!!!!!!!!!!!')
		case '030C':
			pass
		case '201':
		...
		case '300':
			pass
		case _:
			pass  # Default case (if needed)

	return intensity, binary_count


def motif_census_one_graph(G, all_signed_motif_types, motif_types, motif_edges, motif_sms, weight='weight'):
	intensity_df = pd.DataFrame(columns=['motif_type', 'intensity', 'binary_count'])
	for motif_type in motif_types:
		intensity, binary_count = count_motifs_using_matrix(motif_type, G)
		print('I am here!', motif_type, intensity, binary_count)
		intensity_df = pd.concat([intensity_df, pd.DataFrame({'motif_type': [motif_type], 'intensity': [intensity], 'binary_count': [binary_count]})])

	intensity_df = intensity_df.groupby(['motif_type']).sum().reset_index()
	signed_motif_types = intensity_df['motif_type'].unique().tolist()
	intensity_df, _ = add_missing_motif_type(intensity_df, signed_motif_types, all_signed_motif_types)
	return intensity_df.set_index('motif_type').loc[all_signed_motif_types]
...
def plot_motif_intensity_z_scores(intensity_df, path_to_save):
	...
	plt.tight_layout()
	plt.savefig(path_to_save, format='png', dpi=500)
	plt.show()
